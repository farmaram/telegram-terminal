...
TopUser is a single Telegram userbot with two command sets running in the same session.

- Personal Userbot uses the `.` prefix and is shown with `.help`.
- Telegram Terminal uses the `$` prefix and is shown with `$help`.

On startup, TopUser clears the local terminal, shows a small `TopUser running` log area, and sends a short status message to Saved Messages with both help commands.
...
```text
personal-userbot.py                 root launcher
requirements.txt                    root dependencies
TopUser/TopUser.py                  combined userbot entrypoint
TopUser/requirements.txt            TopUser dependencies
TopUser/bot/telegram-terminal.py    embedded Telegram Terminal
```
