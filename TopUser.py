...
def load_font_from_candidates(candidates, size):
    ...
    for candidate in candidates:
        ...
        if font_path.is_file():
            load_value = str(font_path)
        elif isinstance(candidate, str) and not any(separator in candidate for separator in ("/", "\\")):
...
def load_quote_font(size, bold=False):
    return load_font_from_candidates(quote_font_candidates(bold), size)
...
def load_mono_font(size, bold=False):
    return load_font_from_candidates(quote_mono_font_candidates(bold), size)
